 ...
 *
 */
public class Auto_21_3L_Deliver_Left_Switch_Choppy extends CommandGroup {

	private static double ROBOT_TURN_TO_PLATE = -45.0;
	private static double ROBOT_DRIVE_TO_SWITCH_MOVE_1 = 10.0;
	private static double ROBOT_DRIVE_TO_SWITCH_MOVE_2 = 5.5;
	
    public Auto_21_3L_Deliver_Left_Switch_Choppy() {
    	
    	addSequential(new Drive_For_Distance(-0.3, 0.3, 0.0, ROBOT_DRIVE_TO_SWITCH_MOVE_1, 0));	
    	
//    	addSequential(new Drive_Turn_To_Setpoint(0.0, 0.0, ROBOT_TURN_TO_PLATE),1);				//1 Turn to LEFT PLate
//    	addParallel(new Wrist_Down(1));																//2 Prep Wrist
//    	addParallel(new Scissor_To_Setpoint(Scissor.SWITCH,true));								//3 Scissor to Switch Height
//    	addSequential(new Drive_For_Distance(0.0, 0.3, 0.0, ROBOT_DRIVE_TO_SWITCH_MOVE_1,
//    																ROBOT_TURN_TO_PLATE));			//4 Move 1 to Switch
//    	addSequential(new Drive_Turn_To_Setpoint(0.0, 0.0, 0.0),2);								//5 Turn back to Switch
//
//    	//Make sure all Steps are complete
//    	addSequential(new WaitForChildren());
//    	    	
//    	addSequential(new Auto_Util_TimeDelay(.1));													//6 Settle Time
//    	addSequential(new Drive_For_Distance(0.0, 0.3, 0.0, ROBOT_DRIVE_TO_SWITCH_MOVE_2, 0.0),2);	//7 Move to Fence
