...
public class Auto_21_3L_Deliver_Left_Switch extends CommandGroup {
	...
	private static double ROBOT_DRIVE_TO_SWITCH_MOVE_2 = 4.5;
	private static double ROBOT_DRIVE_BACK_AWAY_SWITCH_MOVE = 5.5;//6.25 too far
    ...
    public Auto_21_3L_Deliver_Left_Switch() {
    	...
    	addSequential(new Drive_For_Distance(-0.6, 0.5, 0.0, ROBOT_DRIVE_TO_SWITCH_MOVE_1, 0));	
    	
    	//Make sure all Steps are complete
    	addSequential(new WaitForChildren());
    	
    	addSequential(new Drive_Turn_To_Setpoint(0.0, 0.0, 0.0),.1);								//5 Turn back to Switch

    	//Make sure all Steps are complete
    	addSequential(new WaitForChildren());
    	    	
    	//addSequential(new Auto_Util_TimeDelay(.1));													//6 Settle Time
    	addSequential(new Drive_For_Distance(0.0, 0.4, 0.0, ROBOT_DRIVE_TO_SWITCH_MOVE_2, 0.0),2);	//7 Move to Fence
